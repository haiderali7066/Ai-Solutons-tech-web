...
export const metadata = {
  title: 'AI Privacy & Regulatory Compliance | AI Solution Technologies',
  description: 'Automate enterprise compliance, map shifting global regulations, and secure sensitive data continuously with auditable AI guardrails.',
};
...
══════════════════════════════════════ */
const problems = [
  'Manual compliance audits take months and are outdated the day they are printed',
  'Unstructured data silos containing hidden, unmapped PII and PHI data',
  'Constantly shifting global regulations (GDPR, CCPA, HIPAA, EU AI Act)',
  'Cross-border data transfer violations occurring without executive visibility',
  'Fear of catastrophic compliance fines stalling high-value AI initiatives',
  'Legacy compliance tools flagging thousands of exhausting false positives',
];

const deliverables = [
  'Continuous cross-system privacy & compliance posture assessment',
  'Automated PII/PHI discovery, indexing, and classification engine',
  'Real-time policy violation flagging and remediation workflows',
  'Dynamic regulatory framework mapping (GDPR, CCPA, HIPAA, SOC 2)',
  'AI governance guardrails for internal and consumer-facing models',
  'Instant, audit-ready compliance reporting and telemetry logs',
  'Executive compliance risk and threat vector dashboard',
  'DPO, legal, and compliance team specialized onboarding',
];

const industries = ['Financial Services', 'Healthcare & Life Sciences', 'E-commerce & Retail', 'Tech & SaaS', 'Government & Defense', 'Telecommunications'];
const roles = ['Chief Compliance Officer', 'CISO', 'General Counsel', 'Data Protection Officer', 'Chief Risk Officer'];

/* ══════════════════════════════════════
   MAIN PAGE COMPONENT (100% Server Side)
══════════════════════════════════════ */
export default function AIPrivacyCompliancePage() {
  return (
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="absolute right-[5%] top-[20%] w-64 h-40 bg-white/[0.02] border border-white/10 rounded-xl backdrop-blur-md p-4 anim-float hidden lg:block shadow-2xl">
          ...
          <div className="space-y-2">
            ...
            <div className="w-4/5 h-3 bg-emerald-400/20 rounded-sm"></div>
        ...
        <div className="absolute left-[5%] bottom-[15%] w-48 h-48 bg-white/[0.02] border border-white/10 rounded-full backdrop-blur-md p-4 anim-float-delayed hidden lg:flex items-center justify-center shadow-2xl">
          ...
          <Lock className="absolute w-8 h-8 text-white/50" />
        </div>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            Continuous AI Privacy & Regulatory Compliance 
            <span className="block mt-3 bg-gradient-to-r from-blue-400 via-sky-300 to-indigo-400 bg-clip-text text-transparent pb-2">
              — audit-ready posture, in real time.
            </span>
          </h1>

          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            Map global regulations dynamically onto your complex data systems. Discover sensitive data footprint autonomously, map cross-border flows, and eliminate compliance gaps with air-tight AI precision.
          </p>
        ...
        <div className="flex gap-16 items-center anim-marquee px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {[1, 2, 3].map((set) => (
            <React.Fragment key={set}>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">GDPR Framework</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">CCPA / CPRA</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">HIPAA Shield</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Microsoft Purview</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">OneTrust API</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "Most compliance architectures rely on manual spreadsheets and point-in-time annual audits. We do the exact opposite: <strong className="font-black text-blue-600 relative inline-block">deploy adaptive AI that perpetually scans your enterprise data fabric<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> — instantly pinpointing privacy vulnerabilities and compliance drift. Your legal counsel can sleep soundly because your posture is verified hourly."
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-2/5 relative min-h-[300px] lg:min-h-full bg-slate-900 overflow-hidden">
              <img 
                ...
                alt="Corporate Compliance Assessment" 
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              <BrainCircuit className="w-16 h-16 text-blue-500/10 absolute right-10 top-10 transform group-hover:rotate-12 transition-transform duration-500" />
              <h3 className="text-3xl md:text-4xl font-black text-white font-syne mb-6">
                Cross-Border Health & Fintech Group
              </h3>
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A scaling multinational financial platform dealing with hyper-sensitive client healthcare accounts deployed our AI data compliance safeguards. The AI structural mapping safely discovered over 4.2 million unclassified sensitive PII fields, automatically placing secure data guardrails before a major multi-regional cross-border audit.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    95% <AlertTriangle className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Compliance overhead cut</div>
                  <div className="text-slate-500 text-sm mt-1">by replacing manual risk tracking</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    Zero <ShieldCheck className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Regulatory Violations</div>
                  <div className="text-slate-500 text-sm mt-1">across continuous internal updates</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 grid grid-rows-2 gap-8">
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm h-full">
                ...
                <p className="text-slate-500 text-sm font-light">From framework integration mapping to dynamic runtime go-live</p>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight leading-tight mb-6">
              Ready to de-risk your data footprint?
            </h2>
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Book a 30-minute conversation. We'll map out your regulatory compliance goals, cross-border variables, and explain exactly how real-time AI security guardrails apply to your architecture — with absolute zero commitment.
            </p>
