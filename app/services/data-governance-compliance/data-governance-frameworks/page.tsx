...
export const metadata = {
  title: 'Enterprise Data Governance Frameworks | AI Solution Technologies',
  description: 'Deploy actionable, automated data governance frameworks. Secure your data catalog, define clear lineage, and ensure regulatory compliance.',
};
...
══════════════════════════════════════ */
const problems = [
  'PII and sensitive credentials are exposed in unstructured locations',
  'No clear ownership of critical datasets across business units',
  'Shadow data copies are created without security team oversight',
  'Inability to trace exact data lineage for regulatory audits',
  'Over-privileged user access leading to high data breach risks',
  'AI applications ingestion of unvetted, non-compliant data pools',
];

const deliverables = [
  'Automated data discovery and classification engine',
  'Data catalog deployment & structural business glossary',
  'End-to-end data lineage visualization map',
  'Role-Based and Row-Level security architecture policies',
  'Microsoft Purview implementation & optimization',
  'Data retention, lifecycle, and archiving policies',
  'Continuous compliance and vulnerability monitoring dashboards',
  'Data stewardship operating model & team training',
];

const industries = ['Financial Services', 'Healthcare', 'Government', 'Insurance', 'Energy', 'Retail & E-commerce'];
const roles = ['Chief Data Officer (CDO)', 'Chief Compliance Officer (CCO)', 'VP of Enterprise Data', 'General Counsel', 'CIO'];

/* ══════════════════════════════════════
   MAIN PAGE COMPONENT (100% Server Side)
══════════════════════════════════════ */
export default function DataGovernanceServicePage() {
  return (
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="absolute inset-0 z-0 bg-[#020817]">
          ...
          <img 
            ...
            alt="Secure Enterprise Data Network" 
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            Actionable data governance frameworks built for compliance 
            <span className="block mt-3 bg-gradient-to-r from-blue-400 via-sky-300 to-indigo-400 bg-clip-text text-transparent pb-2">
              — not static policy PDFs.
            </span>
          </h1>

          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            Turn risk management into an automated asset. Implement transparent catalogs, immutable lineage, and ironclad compliance guardrails across your entire cloud data landscape.
          </p>
        ...
        <div className="flex gap-16 items-center anim-marquee px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {[1, 2, 3].map((set) => (
            <React.Fragment key={set}>
              ...
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Data Lineage</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Automated Classification</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">GDPR & CCPA Auditing</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">RBAC & Row-Level Security</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "Most compliance vendors leave you with thousands of pages of unreadable documentation that nobody opens. We do the opposite: <strong className="font-black text-blue-600 relative inline-block">embed governance directly into your technical operations<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> — automating your catalogs, discovery, and object access so security is a natural byproduct of your architecture."
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-2/5 relative min-h-[300px] lg:min-h-full bg-slate-900 overflow-hidden">
              <img 
                ...
                alt="Corporate Audit and Governance" 
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              {/* <FileShield className="w-16 h-16 text-blue-500/10 absolute right-10 top-10 transform group-hover:rotate-12 transition-transform duration-500" /> */}
              <h3 className="text-3xl md:text-4xl font-black text-white font-syne mb-6">
                Healthcare Provider Compliance Overhaul
              </h3>
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A regional healthcare network managing over 2.4 million patient interactions deployed our technical governance blueprint using Microsoft Purview. They automated discovery across active hybrid stores to dynamically quarantine unsecured, sensitive HIPAA vectors.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    75TB <Fingerprint className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Unstructured data mapped</div>
                  <div className="text-slate-500 text-sm mt-1">and classified dynamically</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    100% <CheckCircle2 className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Audit visibility achieved</div>
                  <div className="text-slate-500 text-sm mt-1">on sensitive user touchpoints</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 grid grid-rows-2 gap-8">
              ...
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm">
                ...
                <p className="text-slate-500 text-sm font-light">From initial system scanning to corporate governance launch</p>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight leading-tight mb-6">
              Ready to secure your corporate data foundation?
            </h2>
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Book a 30-minute conversation. We will evaluate your current risk parameters, look into your active ecosystem compliance gaps, and outline a custom framework deployment roadmap at zero cost.
            </p>
