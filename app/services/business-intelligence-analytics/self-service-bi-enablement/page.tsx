...
export const metadata = {
  title: 'Self-Service BI Enablement | AI Solution Technologies',
  description: 'Empower your business units to generate trusted insights instantly. Eliminate the IT dashboard bottleneck with a governed self-service BI framework built on a unified semantic layer.',
};
...
══════════════════════════════════════ */
const problems = [
  'IT and Data teams are bottlenecked with weeks of dashboard requests',
  'Business units export data to isolated Excels, creating broken metrics',
  'Executive reports mismatch because there is no single version of truth',
  'Users do not know which datasets or databases are verified and safe to use',
  'Critical decision-making stalls while waiting for ad-hoc queries',
  'Lack of row-level governance leads to compliance and data leak risks',
];

const deliverables = [
  'Unified semantic layer architecture configuration',
  'Enterprise data cataloging and certified golden datasets',
  'Power BI / Microsoft Fabric workspace and tenant optimization',
  'Role-Based Access Control (RBAC) & Row-Level Security (RLS)',
  'Natural language processing integration for ad-hoc querying',
  'Standardized high-fidelity executive layout templates',
  'Automated data refresh orchestration & drift monitoring',
  'Comprehensive user adoption and self-service enablement guide',
];

const industries = ['Retail & Consumer Goods', 'Manufacturing & Logistics', 'Energy & Utilities', 'Healthcare Systems', 'Financial Services', 'Professional Services'];
const roles = ['CDO', 'CIO', 'VP of Analytics', 'Director of Business Intelligence', 'Head of Operations'];

/* ══════════════════════════════════════
   MAIN PAGE COMPONENT (100% Server Side)
══════════════════════════════════════ */
export default function AISelfServiceBIEnablementPage() {
  return (
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="absolute right-[5%] top-[20%] w-64 h-40 bg-white/[0.02] border border-white/10 rounded-xl backdrop-blur-md p-4 anim-float hidden lg:block shadow-2xl">
          ...
          <div className="space-y-2">
            ...
            <div className="w-3/5 h-3 bg-blue-400/20 rounded-sm"></div>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            Governed Self-Service BI Enablement 
            <span className="block mt-3 bg-gradient-to-r from-blue-400 via-sky-300 to-indigo-400 bg-clip-text text-transparent pb-2">
              — build trusted insights instantly.
            </span>
          </h1>

          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            Transition from an overwhelmed IT ticket backlog to an integrated analytics environment. Empower your lines of business to securely query data directly from single source metrics, fully protected by underlying security controls.
          </p>
        ...
        <div className="flex gap-16 items-center anim-marquee px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {[1, 2, 3].map((set) => (
            <React.Fragment key={set}>
              ...
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">dbt Core</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "Most BI rollouts involve throwing dashboards at users and hoping they learn how to click around. We do the opposite: <strong className="font-black text-blue-600 relative inline-block">establish a bulletproof governed semantic layer first<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> — so that when sales, marketing, or logistics run calculations, they all rely on identical baseline data. The enterprise moves faster because data arguments are eliminated."
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-2/5 relative min-h-[300px] lg:min-h-full bg-slate-900 overflow-hidden">
              <img 
                ...
                alt="Corporate Training Collaboration Room" 
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              <PieChart className="w-16 h-16 text-blue-500/10 absolute right-10 top-10 transform group-hover:rotate-12 transition-transform duration-500" />
              ...
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A massive transport and supply chain provider deployed our governed self-service infrastructure onto Microsoft Fabric. Operations managers across 14 separate regional hubs can now build custom local delay reports dynamically without submitting internal engineering tickets.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    85% <LineChart className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Drop in ad-hoc requests</div>
                  <div className="text-slate-500 text-sm mt-1">freeing technical infrastructure roles</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    4 Hours <CheckCircle2 className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Average time to insight</div>
                  <div className="text-slate-500 text-sm mt-1">down from 3 calendar weeks</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 grid grid-rows-2 gap-8">
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm h-full">
                ...
                <p className="text-slate-500 text-sm font-light">From access governance audit to user certification</p>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight leading-tight mb-6">
              Ready to democratize your data safely?
            </h2>
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Book a 30-minute conversation. We will evaluate your current analytical environment, find your exact data reporting bottlenecks, and map out a clean rollout plan — without obligation.
            </p>
