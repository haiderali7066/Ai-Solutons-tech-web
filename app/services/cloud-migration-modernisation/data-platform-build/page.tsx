...
export const metadata = {
  title: 'Enterprise Data Platform Build | AI Solution Technologies',
  description: 'Design and deploy a scalable, future-proof Modern Data Stack. Unify your data sources into a single source of truth for analytics and AI.',
};
...
══════════════════════════════════════ */
const problems = [
  'Reporting relies on fragile, manual Excel extracts that break constantly',
  'Data engineers spend 80% of their time fixing broken custom ETL scripts',
  'No single source of truth: Finance and Sales report different revenue numbers',
  'Unable to scale analytics due to legacy on-premise database constraints',
  'New data sources take months to integrate into existing reports',
  'Advanced AI and ML initiatives are blocked by dirty, unstructured data',
];

const deliverables = [
  'Cloud data warehouse/lakehouse architecture (Snowflake, Databricks, BigQuery)',
  'Automated ELT pipeline deployment (Fivetran, Airbyte)',
  'Modular data transformation & modeling (dbt)',
  'Data orchestration and scheduling (Airflow, Dagster)',
  'Master data management & quality testing framework',
  'CI/CD deployment for data infrastructure as code',
  'BI tool integration and semantic layer setup',
  'Comprehensive engineering documentation & handover',
];

const industries = ['E-commerce', 'SaaS', 'Healthcare', 'Logistics', 'Fintech', 'Media'];
const roles = ['Head of Data', 'VP of Engineering', 'CTO', 'Lead Data Engineer', 'CIO'];

/* ══════════════════════════════════════
   MAIN PAGE COMPONENT (100% Server Side)
══════════════════════════════════════ */
export default function DataPlatformBuildServicePage() {
  return (
    <main className="overflow-hidden bg-[#fafbfe] text-[#0f172a] font-sans selection:bg-blue-500/30">
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="absolute inset-0 z-0 bg-[#020817]">
          {/* High-quality cloud/infrastructure abstract image */}
          <img 
        ...
        <div className="absolute left-[5%] bottom-[15%] w-48 h-48 bg-white/[0.02] border border-white/10 rounded-full backdrop-blur-md p-4 anim-float-delayed hidden lg:flex items-center justify-center shadow-2xl">
          ...
          <Cloud className="absolute w-8 h-8 text-white/50" />
        </div>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            Build a scalable, future-proof enterprise data platform 
            <span className="block mt-3 bg-gradient-to-r from-blue-400 via-sky-300 to-indigo-400 bg-clip-text text-transparent pb-2">
              — the foundation for real AI.
            </span>
          </h1>

          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            Stop patching broken scripts. We architect and deploy the Modern Data Stack to unify your fragmented systems into a highly governed, automated single source of truth.
          </p>
        ...
        <div className="flex gap-16 items-center anim-marquee px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {[1, 2, 3].map((set) => (
            <React.Fragment key={set}>
              ...
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">dbt</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Fivetran</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
      ...
      <section className="relative py-24 bg-[#fafbfe]">
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <GitBranch className="absolute -right-10 -top-10 w-48 h-48 text-slate-50 -rotate-12 transition-transform duration-700 group-hover:rotate-0 group-hover:scale-110 pointer-events-none" />
            
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "You cannot deploy enterprise AI on top of messy spreadsheets and legacy databases. We build <strong className="font-black text-blue-600 relative inline-block">modern, modular data platforms<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> that ingest your complex sources, transform them automatically, and deliver clean, trusted data to your analysts and algorithms."
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-2/5 relative min-h-[300px] lg:min-h-full bg-slate-900 overflow-hidden">
              <img 
                ...
                alt="Data Visualization and Code" 
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              <Combine className="w-16 h-16 text-blue-500/10 absolute right-10 top-10 transform group-hover:rotate-12 transition-transform duration-500" />
              <h3 className="text-3xl md:text-4xl font-black text-white font-syne mb-6">
                Global Retail Analytics Consolidation
              </h3>
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A rapidly scaling retail brand was relying on manual Excel exports from Shopify, an on-premise ERP, and Zendesk. We built a Modern Data Stack on Snowflake and dbt, automating data pipelines. The result? Executive dashboards update automatically every hour, and their data engineering team now builds ML models instead of fixing broken pipelines.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    95% <Database className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Reduction in reporting time</div>
                  <div className="text-slate-500 text-sm mt-1">from 3 days to 1 hour</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    50+ <CheckCircle2 className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Data sources unified</div>
                  <div className="text-slate-500 text-sm mt-1">into a single reliable warehouse</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 grid grid-rows-2 gap-8">
              ...
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm">
                ...
                <p className="text-slate-500 text-sm font-light">From infrastructure provisioning to live data modeling</p>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Book a 30-minute conversation. We'll review your current data pipelines, discuss your bottlenecks, and outline the architecture for a modern data stack — at no cost and no commitment.
            </p>
