...
export const metadata = {
  title: 'Azure & AWS Cloud Migration | AI Solution Technologies',
  description: 'Securely migrate your enterprise infrastructure to Microsoft Azure or AWS. Zero-downtime migrations, modernized architecture, and continuous cost optimization.',
};
...
══════════════════════════════════════ */
const problems = [
  'Aging on-premise hardware is expensive and risky to maintain',
  'Unpredictable downtime and inadequate disaster recovery',
  'Inability to scale resources quickly during peak business loads',
  'Shadow IT creating fragmented and insecure infrastructure',
  'Fear of migration failure or extended business disruption',
  'Current cloud spending is spiraling out of control',
];

const deliverables = [
  'Comprehensive cloud readiness & dependency assessment',
  'Target architecture design (Azure, AWS, or Multi-Cloud)',
  'Secure, zero-downtime migration strategy',
  'Infrastructure as Code (Terraform/Bicep) automation',
  'Cloud security, compliance, and governance baseline',
  'FinOps setup for continuous cost optimization',
  'Automated disaster recovery and backup implementation',
  'Post-migration managed support & training',
];

const industries = ['Healthcare', 'Retail & E-commerce', 'Financial Services', 'Manufacturing', 'Public Sector', 'SaaS Providers'];
const roles = ['CIO', 'CTO', 'VP of Engineering', 'IT Director', 'CISO'];
    ...
    <main className="overflow-hidden bg-[#fafbfe] text-[#0f172a] font-sans selection:bg-blue-500/30">
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="absolute inset-0 z-0 bg-[#020817]">
          {/* High-quality enterprise/cloud abstract image */}
          <img 
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            Seamless enterprise migration to Azure and AWS
          ...
          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            We don't just "lift and shift." We architect scalable, secure, and cost-optimized cloud environments. Modernize your workloads with full compliance and enterprise governance.
          </p>
        ...
        <div className="flex gap-16 items-center anim-marquee px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {[1, 2, 3].map((set) => (
            <React.Fragment key={set}>
              ...
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Terraform</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Kubernetes</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Azure Migrate</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
            </React.Fragment>
          ))}
        </div>
      ...
      <section className="relative py-24 bg-[#fafbfe]">
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <Server className="absolute -right-10 -top-10 w-48 h-48 text-slate-50 -rotate-12 transition-transform duration-700 group-hover:rotate-0 group-hover:scale-110 pointer-events-none" />
            
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "Most IT providers simply 'lift and shift' your existing mess into the cloud, resulting in bloated costs and legacy issues. We do the opposite: <strong className="font-black text-blue-600 relative inline-block">architect a highly optimized, cloud-native foundation<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> — ensuring rigorous security, massive scalability, and predictable costs from day one."
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              <Cloud className="w-16 h-16 text-blue-500/10 absolute right-10 top-10 transform group-hover:rotate-12 transition-transform duration-500" />
              <h3 className="text-3xl md:text-4xl font-black text-white font-syne mb-6">
                Global Retailer Modernization
              </h3>
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A national retail chain migrated 120+ legacy on-premise servers to a high-availability Azure environment. They modernized their core database layers and achieved flawless 99.99% uptime during their peak holiday sales season, while drastically lowering their total cost of ownership.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    35% <ArrowUpRight className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Reduction in IT costs</div>
                  <div className="text-slate-500 text-sm mt-1">Through FinOps architecture</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    Zero <Activity className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Unplanned downtime</div>
                  <div className="text-slate-500 text-sm mt-1">During final system cutover</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 grid grid-rows-2 gap-8">
              ...
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm">
                ...
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mb-3">Migration Timeline</h4>
                <div className="text-3xl md:text-4xl font-black text-[#020817] font-syne mb-2">6–12 weeks</div>
                <p className="text-slate-500 text-sm font-light">From initial assessment to final production cutover</p>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Book a 30-minute conversation. We'll review your current on-premise or legacy setup, discuss your business goals, and outline a tailored migration path — at no cost and no commitment.
            </p>
