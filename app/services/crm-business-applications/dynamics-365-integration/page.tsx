...
export const metadata = {
  title: 'Enterprise Dynamics 365 Integration | AI Solution Technologies',
  description: 'Connect Microsoft Dynamics 365 natively to your legacy ERPs, custom platforms, and data lakes. High-throughput, secure, and ready for enterprise AI.',
};
...
══════════════════════════════════════ */
const problems = [
  'Dynamics 365 operates as an isolated island from legacy operational data',
  'Out-of-the-box connectors fail under complex business rules and high volume',
  'Spaghetti code and brittle middleware cause frequent synchronization drops',
  'Field service, warehouse, and front-office metrics are perpetually delayed',
  'Manual data reentry between systems driving up labor costs and error rates',
  'Your data foundation is too fragmented to deploy reliable AI agents',
];

const deliverables = [
  'Enterprise integration architecture mapping',
  'Microsoft Dataverse & Azure Integration Services configuration',
  'Custom high-performance API wrappers & middleware adapters',
  'Real-time bi-directional sync pipelines (ERP, CRM, Legacy)',
  'Enterprise Service Bus (Azure Service Bus/Event Grid) implementation',
  'Data validation, transformation, and deduplication logic',
  'Comprehensive end-to-end telemetry and error alert monitoring',
  'Post-go-live optimization and internal IT handoff training',
];

const industries = ['Manufacturing', 'Supply Chain & Logistics', 'Retail & Consumer Goods', 'Healthcare & Life Sciences', 'Professional Services', 'Energy & Utilities'];
const roles = ['CIO', 'CTO', 'VP of Enterprise Applications', 'Director of IT', 'Dynamics 365 Product Owner'];

/* ══════════════════════════════════════
   MAIN PAGE COMPONENT (100% Server Side)
══════════════════════════════════════ */
export default function Dynamics365IntegrationPage() {
  return (
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            High-Performance Dynamics 365 
            <span className="block mt-3 bg-gradient-to-r from-blue-400 via-sky-300 to-indigo-400 bg-clip-text text-transparent pb-2">
              — Zero Silos. Total Pipeline Fluidity.
            </span>
          </h1>

          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            We don’t rely on fragile point-to-point plugins. We architect mission-critical data pathways that unite Dynamics 365 seamlessly with your core legacy systems, supply chains, and third-party infrastructure.
          </p>
        ...
        <div className="flex gap-16 items-center anim-marquee px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {[1, 2, 3].map((set) => (
            <React.Fragment key={set}>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Dynamics 365 FO / CE</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Azure Integration Services</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">SAP ERP & Hana</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Salesforce Architecture</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Custom On-Prem SQL</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "Most systems integrators drop standard logic blocks into your system, collect their fee, and leave you to debug it. We do the opposite: <strong className="font-black text-blue-600 relative inline-block">engineer resilient, multi-point pipelines around your business rules<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> — linking Dynamics 365 to your entire infrastructure with enterprise telemetry. Your data flows cleanly because it was architected defensively."
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-2/5 relative min-h-[300px] lg:min-h-full bg-slate-900 overflow-hidden">
              <img 
                ...
                alt="Industrial Automation and Infrastructure" 
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              <GitMerge className="w-16 h-16 text-blue-500/10 absolute right-10 top-10 transform group-hover:rotate-12 transition-transform duration-500" />
              <h3 className="text-3xl md:text-4xl font-black text-white font-syne mb-6">
                Enterprise Logistics Real-Time Sync
              </h3>
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A heavy logistics operator integrated Dynamics 365 Supply Chain Management with 4 custom proprietary inventory systems and an historical on-prem legacy database. The unified architecture unlocked instant synchronization across state lines.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    &lt; 3 Secs <Activity className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Global Sync Latency</div>
                  <div className="text-slate-500 text-sm mt-1">down from a 12-hour batch delay</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    100% <Layers className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Data Lineage Traceability</div>
                  <div className="text-slate-500 text-sm mt-1">providing perfect system compliance</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 grid grid-rows-2 gap-8">
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm h-full">
                ...
                <p className="text-slate-500 text-sm font-light">From structural audit to production pipeline sign-off</p>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight leading-tight mb-6">
              Ready to unify your enterprise ecosystems?
            </h2>
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Book a 30-minute conversation. We’ll audit your application stack, map out data touchpoints, and construct an actionable implementation framework — completely free of obligation.
            </p>
