...
export const metadata = {
  title: 'Hivenox | Next-Gen Enterprise Workflow Orchestration SaaS',
  description: 'Unify your enterprise operations with Hivenox. Orchestrate data flows, automate complex multi-system handoffs, and monitor performance via autonomous swarm intelligence.',
};
...
══════════════════════════════════════ */
const problems = [
  'Data silos between legacy internal systems and modern SaaS apps',
  'Fragile automation chains that break every time an API updates',
  'Operations teams spending hours manually transferring records',
  'Zero cross-platform visibility on critical business workflows',
  'Process bottlenecks discovered days after causing customer churn',
  'Custom code integrations that require constant engineering upkeep',
];

const deliverables = [
  'Unified cross-system process mapping engine',
  'Autonomous error-detection & self-healing pipelines',
  'Real-time transaction & process tracking graphs',
  'Low-code orchestration designer with enterprise controls',
  'Predictive bottleneck alerting driven by anomaly models',
  'Bank-grade data encryption and secure transmission',
  'Comprehensive security posture & activity audit logs',
  'Full enterprise workspace onboarding & SLA support',
];

const industries = ['Logistics & Supply Chain', 'FinTech & Banking', 'Global Distribution', 'Telecommunications', 'Energy & Utilities', 'Digital Enterprises'];
const roles = ['CIO', 'COO', 'VP of Operations', 'Director of Enterprise Architecture', 'Chief Digital Officer'];

/* ══════════════════════════════════════
   MAIN PAGE COMPONENT (100% Server Side)
══════════════════════════════════════ */
export default function HivenoxSaaSPage() {
  return (
    <main className="overflow-hidden bg-[#fafbfe] text-[#0f172a] font-sans selection:bg-blue-500/30">
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="absolute inset-0 z-0 bg-[#020817]">
          {/* High-quality abstract network structure image */}
          <img 
            src="https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&q=80&w=2000" 
            alt="Connected Infrastructure Grid" 
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            The Swarm Intelligence Platform for 
          ...
          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            Orchestrate complex cross-platform application states, ingest distributed multi-system assets, and achieve real-time operational alignment effortlessly with Hivenox.
          </p>
        ...
        <div className="flex gap-16 items-center anim-marquee px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {[1, 2, 3].map((set) => (
            <React.Fragment key={set}>
              ...
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Salesforce Cloud</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Atlassian Jira API</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Workday Systems</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">ServiceNow Core</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "Most orchestration platforms force your team to construct rigid, linear, fragile automation chains that break constantly. We do the opposite: <strong className="font-black text-blue-600 relative inline-block">deploy an intelligent swarm topology layout<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> — constantly maintaining sync and processing data across structural boundaries with complete contextual integrity."
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-2/5 relative min-h-[300px] lg:min-h-full bg-slate-900 overflow-hidden">
              <img 
                ...
                alt="Cyber Infrastructure Systems" 
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              <Zap className="w-16 h-16 text-blue-500/10 absolute right-10 top-10 transform group-hover:rotate-12 transition-transform duration-500" />
              <h3 className="text-3xl md:text-4xl font-black text-white font-syne mb-6">
                Global Network Real-Time Sync
              </h3>
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A tier-1 telecommunications company integrated Hivenox to stitch operational logs across legacy datacenters and multi-cloud nodes. The enterprise managed to eliminate workflow delivery lag entirely.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    400M+ <Activity className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Daily Jobs Processed</div>
                  <div className="text-slate-500 text-sm mt-1">with zero infrastructure failure</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    14x <BrainCircuit className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Faster Handoff Speed</div>
                  <div className="text-slate-500 text-sm mt-1">across multi-layered endpoints</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-7 rounded-2xl bg-[#fafbfe] border border-slate-200 p-8 md:p-12 shadow-sm transition-shadow hover:shadow-md">
              <h3 className="text-xl font-bold text-[#020817] mb-10 uppercase tracking-wide flex items-center gap-3">
                <Users className="w-5 h-5 text-blue-600" /> Target Profile
              </h3>
            ...
            {/* Logistics (Onboarding Timeframe) */}
            <div className="lg:col-span-5 grid grid-rows-1">
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm h-full">
                ...
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mb-3">Enterprise Deployment</h4>
                <div className="text-3xl md:text-4xl font-black text-[#020817] font-syne mb-2">2–4 weeks</div>
                <p className="text-slate-500 text-sm font-light">From core asset setup to sandbox execution validation</p>
              </div>
            </div>
      ...
      <section className="pb-24 px-6 bg-[#fafbfe]">
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            <span className="text-xs font-bold tracking-widest text-blue-200 uppercase mb-4 block">Strategic Architecture Briefing</span>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight leading-tight mb-6">
              Ready to unify your system orchestration layer?
            </h2>
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Connect with our solutions engineering team. We'll evaluate your integration topology map, diagnose current throughput blocks, and assemble a customized proof-of-concept deployment plan.
            </p>
