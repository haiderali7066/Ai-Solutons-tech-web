...
export const metadata = {
  title: 'AI Security Access Control & Identity Governance | AI Solution Technologies',
  description: 'Deploy autonomous identity governance and access control. Automate least-privilege enforcement, flag access drift, and secure your enterprise stack with full auditability.',
};
...
══════════════════════════════════════ */
const problems = [
  'Over-privileged accounts creating a massive corporate attack surface',
  'Manual quarterly access reviews take weeks and miss critical permission drift',
  'Delayed or incomplete offboarding leaving doors open for former employees',
  'Security teams flooded with false-positive anomalous access alerts',
  'Insider threats go unnoticed due to fragmented, siloed identity logs',
  'Audit compliance (SOC2/ISO) requires chaotic, manual screenshot gathering',
];

const deliverables = [
  'Cross-system identity & access vector assessment',
  'AI-powered automated provisioning & deprovisioning engine',
  'Real-time access drift and anomaly detection',
  'Zero-Trust least privilege scoring system',
  'One-click automated offboarding workflows',
  'Immutable, cryptographic audit logs for access decisions',
  'Executive security posture and risk dashboard',
  'SecOps and compliance team onboarding',
];

const industries = ['Government & Defense', 'Financial Services', 'Healthcare & Life Sciences', 'Energy & Utilities', 'SaaS & Technology', 'Critical Infrastructure'];
const roles = ['CISO', 'CIO', 'VP of Enterprise Infrastructure', 'Head of IAM', 'Director of Cyber Security'];

/* ══════════════════════════════════════
   MAIN PAGE COMPONENT (100% Server Side)
══════════════════════════════════════ */
export default function AISecurityAccessControlPage() {
  return (
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="absolute right-[5%] top-[20%] w-64 h-40 bg-white/[0.02] border border-white/10 rounded-xl backdrop-blur-md p-4 anim-float hidden lg:block shadow-2xl">
          ...
          <div className="space-y-2">
            ...
            <div className="w-4/5 h-3 bg-emerald-500/20 rounded-sm"></div>
        ...
        <div className="absolute left-[5%] bottom-[15%] w-48 h-48 bg-white/[0.02] border border-white/10 rounded-full backdrop-blur-md p-4 anim-float-delayed hidden lg:flex items-center justify-center shadow-2xl">
          ...
          <Fingerprint className="absolute w-8 h-8 text-white/50" />
        </div>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            AI-Driven Security Access Control
            <span className="block mt-3 bg-gradient-to-r from-blue-400 via-sky-300 to-indigo-400 bg-clip-text text-transparent pb-2">
              — Zero Trust, enforced autonomously.
            </span>
          </h1>

          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            Enforce absolute least-privilege posture across your entire infrastructure. Continuously analyze behavior, map access drift, and isolate malicious identity deviations instantly without disrupting operations.
          </p>
        ...
        <div className="flex gap-16 items-center anim-marquee px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {[1, 2, 3].map((set) => (
            <React.Fragment key={set}>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Okta / Auth0</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Microsoft Entra ID</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">AWS IAM</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">CyberArk</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Ping Identity</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "Traditional access management relies on static rules that are obsolete hours after an audit. We do the opposite: <strong className="font-black text-blue-600 relative inline-block">embed AI continuously inside your identity architecture<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> — tracking risk vectors and executing micro-revocations in real-time. The CISO can confidently prove compliance because every policy change is cryptographically backed."
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              <Lock className="w-16 h-16 text-blue-500/10 absolute right-10 top-10 transform group-hover:rotate-12 transition-transform duration-500" />
              ...
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A regional utilities group managing interconnected grid systems deployed our AI security access controls across their hybrid identity perimeter. The platform processed millions of authentication logs daily, discovering hidden drift patterns and containing active structural risks automatically.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    94% <ShieldAlert className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Access Review Automation</div>
                  <div className="text-slate-500 text-sm mt-1">eliminating manual configuration bloat</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    &lt; 2 Hrs <CheckCircle2 className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Orphaned Admin Mitigation</div>
                  <div className="text-slate-500 text-sm mt-1">flagged and isolated instantly</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 grid grid-rows-2 gap-8">
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm h-full">
                ...
                <p className="text-slate-500 text-sm font-light">From initial identity mapping to global system enforcement</p>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight leading-tight mb-6">
              Ready to secure your network perimeter with AI?
            </h2>
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Book a 30-minute conversation. We will review your current access workflows, evaluate identity silo layouts, and structure a custom proof-of-concept design architecture — without cost or obligation.
            </p>
