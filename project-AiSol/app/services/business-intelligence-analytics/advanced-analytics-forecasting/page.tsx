...
export const metadata = {
  title: 'Advanced Analytics & Predictive Forecasting | AI Solution Technologies',
  description: 'Shift from reactive to proactive. Deploy enterprise time-series forecasting, predictive modeling, and real-time operational analytics engines built on your live data streams.',
};
...
══════════════════════════════════════ */
const problems = [
  'Business plans are based on trailing metrics and backward-looking data',
  'Demand planning relies heavily on manual adjustments and subjective "gut feel"',
  'Supply chain shocks and market shifts are reacted to instead of anticipated',
  'High inventory overhead and stockouts caused by inaccurate seasonality mapping',
  'Siloed operational data prevents a unified view of emerging market trends',
  'Complex predictive scenario modeling takes weeks instead of minutes',
];

const deliverables = [
  'Deep data ingestion pipeline (Historical ERP, CRM, IoT, Market Indicators)',
  'Enterprise time-series forecasting models (Demand, Revenue, Capacity)',
  'Multi-variable predictive algorithms with feature importance visibility',
  'Dynamic simulation sandboxes ("What-if" scenario modeling tools)',
  'Automated variance and anomaly detection alerting engines',
  'Interactive, executive-grade predictive dashboards',
  'Continuous machine learning model drift monitoring & auto-retraining',
  'Operational team alignment and predictive training',
];

const industries = ['Manufacturing', 'Supply Chain & Logistics', 'Retail & E-commerce', 'Energy & Utilities', 'Financial Services', 'Infrastructure'];
const roles = ['Chief Data Officer (CDO)', 'VP of Supply Chain', 'VP of Operations', 'CFO', 'Chief Growth Officer'];

/* ══════════════════════════════════════
   MAIN PAGE COMPONENT (100% Server Side)
══════════════════════════════════════ */
export default function AIAdvancedAnalyticsPage() {
  return (
    <main className="overflow-hidden bg-[#fafbfe] text-[#0f172a] font-sans selection:bg-blue-500/30">
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="absolute inset-0 z-0 bg-[#020817]">
          {/* High-quality advanced analytics/forecasting abstract image */}
          <img 
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            ...
            <span className="block mt-3 bg-gradient-to-r from-blue-400 via-sky-300 to-indigo-400 bg-clip-text text-transparent pb-2">
              — drop the guesswork. Trust your trajectory.
            </span>
          </h1>

          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            Shift your leadership stack from historical reporting to forward-looking execution. Deploy high-fidelity forecasting engines to simulate market scenarios, optimize inventory, and uncover margin drivers.
          </p>
        ...
        <div className="flex gap-16 items-center anim-marquee px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {[1, 2, 3].map((set) => (
            <React.Fragment key={set}>
              ...
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">AWS Redshift</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Power BI Premium</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "Most BI platforms merely look backward to show you how you failed or succeeded last month. We build the opposite: <strong className="font-black text-blue-600 relative inline-block">systems that isolate variables to plot exactly what happens next<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> — across your logistics, distribution, and product volumes. Run recursive scenarios with parameters you control."
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-2/5 relative min-h-[300px] lg:min-h-full bg-slate-900 overflow-hidden">
              <img 
                ...
                alt="Industrial Supply Chain and Operations" 
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              ...
              <h3 className="text-3xl md:text-4xl font-black text-white font-syne mb-6">
                Multi-State Industrial Logistics Provider
              </h3>
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A heavy logistics operator integrated dynamic multi-variable demand modeling across 14 distribution networks. By connecting regional weather indices, historical inventory lags, and macroeconomic data streams, their operational leads automated scheduling ahead of bottleneck anomalies.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    34% <TrendingUp className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Increase in Forecast Accuracy</div>
                  <div className="text-slate-500 text-sm mt-1">over traditional spreadsheet curves</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    $1.2M <BarChart3 className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Overhead Prevented</div>
                  <div className="text-slate-500 text-sm mt-1">in dead inventory within 8 months</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 grid grid-rows-2 gap-8">
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm h-full">
                ...
                <p className="text-slate-500 text-sm font-light">From core feature evaluation to live dashboard deployment</p>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight leading-tight mb-6">
              Ready to construct predictive operational frameworks?
            </h2>
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Book a 30-minute conversation. We'll map your existing data landscape, trace structural gaps in your active pipeline, and configure a strategic blueprint for algorithmic forecasting models — completely cost and commitment free.
            </p>
