...
export const metadata = {
  title: 'Modern ETL/ELT Services | AI Solution Technologies',
  description: 'Migrate, transform, and load your enterprise data with high-performance ETL/ELT pipelines built for modern cloud data warehouses.',
};
...
══════════════════════════════════════ */
const problems = [
  'Data pipelines are brittle and constantly breaking',
  'Overnight batch jobs regularly spill into business hours',
  'Writing custom scripts for every new data source',
  'Engineers spend 80% of their time fixing errors, not building',
  'High compute costs from inefficient data transformations',
  'Inconsistent metrics across different downstream reports',
];

const deliverables = [
  'Current-state legacy pipeline architecture assessment',
  'Migration to modern ELT (dbt, Snowflake, Databricks)',
  'Automated orchestration (Airflow, Databricks Workflows, ADF)',
  'Data quality testing and anomaly detection layers',
  'Zero-downtime historical data migration',
  'Incremental processing and Change Data Capture (CDC)',
  'CI/CD deployment setup for data transformations',
  'Comprehensive engineering documentation & runbooks',
];

const industries = ['Retail & E-commerce', 'Financial Services', 'Healthcare', 'SaaS', 'Manufacturing', 'Telecommunications'];
const roles = ['Head of Data Engineering', 'CDO', 'CTO', 'VP of Analytics', 'Data Architect'];

/* ══════════════════════════════════════
   MAIN PAGE COMPONENT (100% Server Side)
══════════════════════════════════════ */
export default function ETLELTServicesPage() {
  return (
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="absolute right-[5%] top-[20%] w-64 h-40 bg-white/[0.02] border border-white/10 rounded-xl backdrop-blur-md p-4 anim-float hidden lg:block shadow-2xl">
          ...
          <div className="space-y-2">
            ...
            <div className="w-full h-3 bg-blue-500/30 rounded-sm"></div>
          </div>
        </div>
        ...
        <div className="absolute left-[5%] bottom-[15%] w-48 h-48 bg-white/[0.02] border border-white/10 rounded-full backdrop-blur-md p-4 anim-float-delayed hidden lg:flex items-center justify-center shadow-2xl">
          ...
          <Settings2 className="absolute w-8 h-8 text-white/50" />
        </div>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            ...
            <span className="block mt-3 bg-gradient-to-r from-blue-400 via-sky-300 to-indigo-400 bg-clip-text text-transparent pb-2">
              built for scale, not failure.
            </span>
          </h1>

          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            Stop firefighting brittle data jobs. We architect high-performance, automated pipelines that extract reliably, load instantly, and transform data using software engineering best practices.
          </p>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "Stop maintaining brittle spaghetti code. We design modern ELT architectures where data is extracted reliably, loaded efficiently, and <strong className="font-black text-blue-600 relative inline-block">transformed using software engineering best practices<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> — turning your data team into a high-throughput factory, not a daily fire department."
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              ...
              <h3 className="text-3xl md:text-4xl font-black text-white font-syne mb-6">
                E-Commerce Enterprise Migration
              </h3>
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A global retailer struggled with legacy SSIS packages that took 14 hours to run, severely impacting daily reporting. We modernized their stack using Fivetran, Snowflake, and dbt. The new ELT pipeline runs incrementally in minutes, enabling real-time inventory analytics.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    14h to 15m <Timer className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Run Time Reduction</div>
                  <div className="text-slate-500 text-sm mt-1">from daily batch to near real-time</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    40% <TrendingDown className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Compute Savings</div>
                  <div className="text-slate-500 text-sm mt-1">via incremental dbt models</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 grid grid-rows-2 gap-8">
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm h-full">
                ...
                <p className="text-slate-500 text-sm font-light">From legacy assessment to fully orchestrated pipelines</p>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Book a 30-minute architecture review. We'll examine your current ETL setup, discuss your scalability bottlenecks, and outline a modernization roadmap — at no cost and no commitment.
            </p>
