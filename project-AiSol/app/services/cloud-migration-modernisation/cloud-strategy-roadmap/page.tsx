...
export const metadata = {
  title: 'Enterprise Cloud Strategy & Roadmap | AI Solution Technologies',
  description: 'Design a high-performance, cost-optimized enterprise cloud strategy. Modernize infrastructure, orchestrate multi-cloud ecosystems, and secure your migration roadmap.',
};
...
══════════════════════════════════════ */
const problems = [
  'Cloud spend is spinning out of control with zero architectural visibility',
  'Legacy applications are stuck in costly, unoptimized "lift-and-shift" configurations',
  'Lack of a unified multi-cloud governance and compliance framework',
  'Security vulnerabilities emerging across fragmented cloud environments',
  'Severe vendor lock-in limiting operational agility and bargaining leverage',
  'Engineering teams wasting months provisioning bespoke, unscalable infrastructure',
];

const deliverables = [
  'Current-state infrastructure and cloud asset discovery audit',
  'Total Cost of Ownership (TCO) and multi-year ROI projection models',
  'Multi-cloud architecture blueprint (AWS, Azure, GCP matching criteria)',
  'Cloud-native application modernization strategy (Containers, Serverless)',
  'Enterprise security, landing zone, and identity governance guardrails',
  'Automated Infrastructure-as-Code (IaC) templates and deployment playbooks',
  'Phased application migration roadmap with detailed risk mitigation plans',
  'Cloud Center of Excellence (CCoE) operational readiness and team training',
];

const industries = ['Financial Services', 'Manufacturing', 'Logistics & Supply Chain', 'Healthcare & Pharma', 'Retail & E-commerce', 'Energy & Utilities'];
const roles = ['CIO', 'CTO', 'VP of Infrastructure', 'Head of Cloud Engineering', 'Chief Information Security Officer'];

/* ══════════════════════════════════════
   MAIN PAGE COMPONENT (100% Server Side)
══════════════════════════════════════ */
export default function CloudStrategyRoadmapPage() {
  return (
    <main className="overflow-hidden bg-[#fafbfe] text-[#0f172a] font-sans selection:bg-blue-500/30">
      ...
      <section className="relative overflow-hidden pt-32 pb-24 md:pt-48 md:pb-32 border-b border-white/5">
        ...
        <div className="absolute inset-0 z-0 bg-[#020817]">
          {/* High-quality abstract network/cloud enterprise image */}
          <img 
            src="https://images.unsplash.com/photo-1600132806370-bf17e65e942f?auto=format&fit=crop&q=80&w=2000" 
            alt="Cloud Network Grid" 
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] tracking-tight text-white font-syne">
            Enterprise-Grade Cloud Strategy & Roadmaps
            <span className="block mt-3 bg-gradient-to-r from-blue-400 via-sky-300 to-indigo-400 bg-clip-text text-transparent pb-2">
              — engineered for performance, scale, and ROI.
            </span>
          </h1>

          <p className="mt-8 max-w-3xl mx-auto text-lg md:text-xl leading-relaxed text-slate-300 font-light">
            Stop letting cloud bills dictate your margins. Align multi-cloud infrastructure, automate modern landing zones, and design an authoritative migration path built on rigid governance and proven design paradigms.
          </p>
        ...
        <div className="flex gap-16 items-center anim-marquee px-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {[1, 2, 3].map((set) => (
            <React.Fragment key={set}>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Amazon Web Services</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Microsoft Azure</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Google Cloud Platform</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Terraform / IaC</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span className="text-sm font-bold tracking-widest text-slate-600 uppercase">Kubernetes / K8s</span>
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
            </React.Fragment>
          ))}
        </div>
      ...
      <section className="relative py-24 bg-[#fafbfe]">
        <div className="max-w-4xl mx-auto px-6">
          <div className="group rounded-2xl bg-white border border-slate-200 p-10 md:p-14 text-center shadow-[0_10px_40px_rgba(15,23,42,0.03)] relative overflow-hidden transition-all duration-500 hover:border-blue-200 hover:shadow-[0_20px_50px_rgba(37,99,235,0.08)]">
            ...
            <Layers className="absolute -right-10 -top-10 w-48 h-48 text-slate-50 -rotate-12 transition-transform duration-700 group-hover:rotate-0 group-hover:scale-110 pointer-events-none" />
            
            <p className="relative z-10 text-xl md:text-2xl leading-relaxed text-[#020817] font-light font-syne">
              "Most systems integrators will push you toward a lazy, automated 'lift-and-shift' migration that multiplies your operating costs. We do the opposite: <strong className="font-black text-blue-600 relative inline-block">architect an optimized hybrid and multi-cloud topology<span className="absolute bottom-0 left-0 w-full h-1 bg-blue-200 -z-10 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></span></strong> — rightsizing assets, integrating automated FinOps, and ensuring compliance before single application migrations ever begin."
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="rounded-3xl overflow-hidden bg-[#020817] flex flex-col lg:flex-row border border-slate-800 shadow-2xl relative group">
            ...
            <div className="lg:w-3/5 p-10 md:p-16 relative z-10">
              <Server className="w-16 h-16 text-blue-500/10 absolute right-10 top-10 transform group-hover:rotate-12 transition-transform duration-500" />
              <h3 className="text-3xl md:text-4xl font-black text-white font-syne mb-6">
                Multi-National Enterprise Architecture Transition
              </h3>
              <p className="text-lg text-slate-300 leading-relaxed font-light mb-10">
                A massive manufacturing and distribution corporate entity (operating across 40+ legacy facilities and fragmented on-premises systems) executed our phased cloud architecture strategy to modernize legacy workloads into automated AWS and Azure landings.
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    34% Cut <Cpu className="w-5 h-5 text-emerald-400 transform group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">In Infrastructure Spend</div>
                  <div className="text-slate-500 text-sm mt-1">reclaimed inside the first fiscal quarter</div>
                </div>
                <div className="group/stat">
                  <div className="text-4xl font-black text-white mb-2 flex items-center gap-2">
                    0 Incidents <ShieldCheck className="w-5 h-5 text-emerald-400 group-hover/stat:scale-110 transition-transform" />
                  </div>
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">Downtime or Compromise</div>
                  <div className="text-slate-500 text-sm mt-1">across continuous phased migrations</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 grid grid-rows-2 gap-8">
              <div className="rounded-2xl bg-white border border-slate-200 p-8 flex flex-col justify-center relative overflow-hidden group shadow-sm h-full">
                ...
                <p className="text-slate-500 text-sm font-light">From infrastructure assessment to comprehensive roadmap signoff</p>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto relative bg-gradient-to-br from-[#1e40af] via-[#2563eb] to-[#3b82f6] rounded-[2.5rem] p-10 sm:p-16 text-center overflow-hidden shadow-[0_40px_100px_rgba(37,99,235,0.28)]">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight leading-tight mb-6">
              Ready to construct your modern enterprise cloud roadmap?
            </h2>
            <p className="text-white/80 text-base max-w-2xl mx-auto mb-10 font-light">
              Book a 30-minute infrastructure advisory evaluation. We will break down your operational bottlenecks, audit scaling concerns, and frame an actionable high-efficiency approach — completely free of friction and binding commitments.
            </p>
